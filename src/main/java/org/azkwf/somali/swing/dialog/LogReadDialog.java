...
public class LogReadDialog extends ConfigDialog<LogReadConfig> {
	...
	public LogReadDialog(final JFrame parent) {
		super(parent);
		setTitle("ログファイル読込み");
	...
	protected LogReadConfig getConfig() {
		...
		config.setDateFrom(toDate(txtDateFrom.getText()));
		config.setDateTo(toDate(txtDateTo.getText()));
	...
	protected void doInit(final JPanel client) {
		...
		txtDateFrom = new JTextField("2016/01/01 00:00:00");
		lblDateFromTo = new JLabel("〜");
		txtDateTo = new JTextField("2016/12/01 00:00:00");
		// Level
		...
		txtLogFile.setLocation(MARGIN + lblLogFile.getWidth() + SPACE, y);
		btnLogFile.setSize(100, HEIGHT);
		// Date
		y += HEIGHT + SPACE + 10;
		lblDate.setLocation(MARGIN, y);
		lblDate.setSize(80, HEIGHT);
		txtDateFrom.setLocation(lblDate.getX() + lblDate.getWidth() + SPACE, y);
		...
		txtLevel.setLocation(lblLevel.getX() + lblLevel.getWidth() + SPACE, y);
		txtLevel.setSize(100, HEIGHT);
		// Logger
		y += HEIGHT + SPACE;
		lblLogger.setLocation(MARGIN, y);
		lblLogger.setSize(80, HEIGHT);
		txtLogger.setLocation(lblLogger.getX() + lblLogger.getWidth() + SPACE, y);
		txtLogger.setSize(300, HEIGHT);
		// Message
		y += HEIGHT + SPACE;
		lblMessage.setLocation(MARGIN, y);
		lblMessage.setSize(80, HEIGHT);
		txtMessage.setLocation(lblMessage.getX() + lblMessage.getWidth() + SPACE, y);
	...
	protected void doResize(final JPanel client) {
		...
		int y = MARGIN;
		txtLogFile.setSize(width - (MARGIN + lblLogFile.getWidth() + SPACE + SPACE + btnLogFile.getWidth() + MARGIN),
				HEIGHT);
